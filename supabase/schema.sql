...
create table if not exists public.consorcios (
  ...
  cuit text,
  tipo text not null default 'edificio' check (tipo in ('edificio', 'barrio_privado', 'country', 'otro')),
  tipo_otro text,
...
create table if not exists public.padron_accesos_importados (
  ...
  dni text,
  unidad_funcional text,
  puesto_vigilancia text,
  codigo_acceso text,
  codigo_acceso_expires_at timestamptz,
  codigo_reclamado_at timestamptz,
  codigo_reclamado_por uuid references auth.users(id) on delete set null,
  es_menor boolean not null default false,
  adulto_responsable_email text,
