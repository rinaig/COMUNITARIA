...
function getStatusClassName(status: DeliveryStatus) {
  ...
  if (status === "omitido") {
    return "rounded-full bg-slate-200 px-3 py-2 text-xs font-extrabold uppercase tracking-[0.18em] text-slate-600";
  }

  return "status-badge status-badge--warning";
}

function getNetworkAwareMessage(error: unknown, fallback: string) {
  ...
  if (lowerMessage.includes("failed to fetch") || lowerMessage.includes("fetch failed") || lowerMessage.includes("network")) {
    return "No se pudo contactar el procesador saliente desde el navegador. Revisa la sesion, el entorno server-side o la conectividad local.";
  }
...
export function AdminOutboundDeliveriesPanel() {
  ...
  const loadRows = useCallback(async () => {
    ...
    const [rowsResult, integrationsResult] = await Promise.all([
      supabase
      ...
      supabase
        ...
        .select("id, canal, proveedor, remitente, modo_prueba, activo")
    ...
    setRows((rowsResult.data as DeliveryRow[] | null) ?? []);
    setIntegrations((integrationsResult.data as IntegrationRow[] | null) ?? []);
    setLoading(false);
  }, [supabase]);
  ...
  async function handleProcessQueue() {
    ...
    try {
      ...
      setError(getNetworkAwareMessage(processError, "No se pudo procesar la cola saliente."));
      setProcessingQueue(false);
      return;
    }
  ...
  const queueSummary = rows.reduce(
    (accumulator, row) => {
      accumulator[row.estado] += 1;
      return accumulator;
    },
    { pendiente: 0, enviado: 0, fallido: 0, omitido: 0 },
  );

  const integrationsByChannel = {
    email: integrations.find((item) => item.canal === "email") ?? null,
    whatsapp: integrations.find((item) => item.canal === "whatsapp") ?? null,
  } satisfies Record<Channel, IntegrationRow | null>;

  function getChannelReadiness(channel: Channel) {
    const row = integrationsByChannel[channel];

    if (!row) {
      return {
        label: "sin configurar",
        detail: "Todavia no hay credenciales cargadas para este canal.",
    ...
    if (!row.activo) {
      return {
        label: "inactivo",
        detail: "La configuracion existe pero el canal esta desactivado.",
        className: "status-badge status-badge--neutral",
      };
    }
    ...
    if (row.modo_prueba) {
      return {
        label: "modo prueba",
        detail: `Canal activo con proveedor ${row.proveedor || "sin proveedor"}, pero sin despacho real.`,
    ...
    return {
      label: "listo",
      detail: `Canal activo para envio real con proveedor ${row.proveedor || "sin proveedor"}.`,
  ...
  return (
    <section className="mt-6 glass-panel rounded-[2rem] p-8">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article className="metric-card"><p className="text-sm uppercase tracking-[0.18em] text-slate-500">Pendientes</p><p className="mt-3 text-3xl font-semibold text-slate-950">{queueSummary.pendiente}</p></article>
        <article className="metric-card"><p className="text-sm uppercase tracking-[0.18em] text-slate-500">Enviadas</p><p className="mt-3 text-3xl font-semibold text-slate-950">{queueSummary.enviado}</p></article>
        <article className="metric-card"><p className="text-sm uppercase tracking-[0.18em] text-slate-500">Fallidas</p><p className="mt-3 text-3xl font-semibold text-slate-950">{queueSummary.fallido}</p></article>
        <article className="metric-card"><p className="text-sm uppercase tracking-[0.18em] text-slate-500">Omitidas</p><p className="mt-3 text-3xl font-semibold text-slate-950">{queueSummary.omitido}</p></article>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {(["email", "whatsapp"] as Channel[]).map((channel) => {
          ...
          return <article className="role-card" key={channel}><div className="flex items-start justify-between gap-3"><div><p className="text-sm uppercase tracking-[0.2em] text-slate-500">{channelLabels[channel]}</p><p className="mt-2 text-sm leading-7 text-slate-600">{readiness.detail}</p></div><span className={readiness.className}>{readiness.label}</span></div><div className="mt-3 flex flex-wrap gap-3 text-xs uppercase tracking-[0.18em] text-slate-400"><span>{integration?.proveedor ?? "sin proveedor"}</span><span>{integration?.remitente ?? "sin remitente visible"}</span></div></article>;
        })}
      </div>
